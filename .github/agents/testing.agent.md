...
6. Start from the post-lint state and identify the affected source and test files.
7. Use the `/testing` skill for repo-specific Jest and React Testing Library patterns.
8. If the task touches an external library, framework, or API, use the `/internet-research` workflow first:
   - Call `resolve-library-id` for each relevant library.
   - Call `get-library-docs` for the exact topic in question.
   - Summarize the relevant behavior before writing tests.
9. Create or update tests following `.github/copilot-instructions.md`.
10. Prefer focused runs first:
...
13. If failures persist after 3 attempts, record the remaining failures and stop.
14. When new behavior is added, add or update the corresponding test coverage.
15. Once all tests pass, run `npm test -- --ci --coverage` to verify the coverage gate.
16. If coverage drops below **80%** on any metric (branches, functions, lines, statements):
